...
The way you find conformance issues for an API specification depends on whether the API is governed through an active profile in API Governance. 

To find and fix an API specification's conformance:

. Initiate validation of API specifications against rulesets in:
+
...
+
* For an API validated using active profiles in API Governance, view conformance information in the API's:
+
** <<view-spec-issues-in-report, Validation report in API Governance>>
** xref:view-conformance-status-in-exchange.adoc[Conformance Status page in Exchange]
+ 
* For an API validated outside of API Governance, view conformance information in:
+
** <<project-errors,Design Center>>
** <<validate-in-cli,Reports generated through CLI>>
+
. Fix the issues using <<project-errors,Design Center>>. 
