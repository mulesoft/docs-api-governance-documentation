...
Use the following example sequence to get started validating you ruleset, generating documentation, and uploading your ruleset to Exchange. Replace the details, such as the folder name, ruleset file name, and ruleset documentation ZIP file name, with your own.
...
----
[calloutlist]
.. Validates the ruleset locally. The ruleset file name is `myruleset.yaml` and it's in the `~/myRulesetFolder/` folder.
...
.. Uploads the ruleset, `myruleset.yaml`, and its documentation, `~/myRulesetFolder/ruleset.doc.zip`. Both files are in the `~/myRulesetFolder/` folder. 
