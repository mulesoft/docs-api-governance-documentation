...
[NOTE]
====
This section describes features available only in the enhanced MuleSoft experience. The *Governance Strategies* page is not available in the classic Anypoint Platform UI. See xref:general::exp-governance-work-with-strategies.adoc[].
====

If you have connected third-party gateway providers (Kong, Apigee, AWS API Gateway, or Akamai), the *Governance Strategies* page provides a unified conformance view across all gateways and MuleSoft-native APIs. Conformance for all services — regardless of their provider — appears in the same governance strategy report.

To view cross-gateway conformance:

. In *Governance* > *Governance Strategies*, select an active governance strategy that targets multiple providers or uses the *All runtimes* scope.
. Review the conformance summary, which lists each governed service alongside its provider or gateway.
. To filter by a specific provider, use the *Provider* column or filter controls in the strategy view.

NOTE: Services with read-only scanner credentials appear with a *Read-Only* policy status indicator. Conformance validation still runs for these services, but policy application requires updated credentials. See xref:find-and-fix-conformance-issues.adoc#update-scanner-credentials[Update Scanner Credentials].
...
* Number of nonconformant APIs (fail)
* Timestamp
* Gateway/Provider (for APIs discovered from connected third-party providers such as Kong, Apigee, AWS API Gateway, or Akamai; blank for MuleSoft-native APIs)
* Policy Application Status (Enabled or Read-Only; indicates whether policies can be programmatically applied to the service)
...
* Number of nonconformant APIs (fail)
* Timestamp
* Gateway/Provider (for APIs on connected third-party providers; blank for MuleSoft-native APIs)
...
[[gov-cross-gateway-export]]
=== Cross-Gateway Policy Audit Export

[NOTE]
====
This export is available only in the enhanced MuleSoft experience via *Governance* > *Governance Strategies*. See xref:general::exp-governance-work-with-strategies.adoc[].
====

For compliance purposes (such as SOC 2 or GDPR audits), you can export a cross-gateway policy audit report that consolidates policy application evidence across all connected providers and MuleSoft-native APIs.

To export a cross-gateway policy audit report:

. In *Governance* > *Governance Strategies*, select the governance strategy that targets the gateways or providers you want to audit.
. Click *Export to CSV*.
+
The exported CSV file contains policy audit evidence for each service governed by the selected strategy. The information includes:
+
* Organization ID
* API ID
* API Version
...
NOTE: Services with a *Read-Only* policy application status appear in the audit report but indicate that policies cannot be applied programmatically. To remediate, update the scanner credentials for the associated provider. See xref:find-and-fix-conformance-issues.adoc#update-scanner-credentials[Update Scanner Credentials].
