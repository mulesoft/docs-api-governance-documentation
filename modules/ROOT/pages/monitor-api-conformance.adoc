...
// AUG 13 (POLICY WRITE SCOPE): Policy Application Status (Enabled/Read-Only) reflects write_capable field.
...
// AUG 13 (POLICY WRITE SCOPE): Uncomment when Policy Write ships Aug 13.
...
// READ PHASE SCOPE QUESTION: What data does the cross-gateway policy audit export surface in the Read phase (7/16)?
// Does it show policies already existing on third-party gateways (discovered via scanning), or only policies applied via MuleSoft (Write phase, Aug 13)?
// "Policy application evidence" may be premature if gateway-applied policies are a Write-phase feature. Confirm with engineering before publishing.
