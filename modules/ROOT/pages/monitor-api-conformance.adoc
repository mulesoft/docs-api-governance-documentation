...
== Evaluating API Conformance Using the CLI

Those who do not have access to API Governance, such as developers, can use the CLI to evaluate which rulesets their APIs might be subject to validation against based on given filters. This enables developers to be proactive in understanding their rulesets' potential conformance to rulesets before their APIs become governed. 
