...
If you have connected third-party gateway providers (Kong, Apigee, AWS API Gateway, or Akamai), API Governance provides a unified conformance view across all gateways and MuleSoft-native APIs in a single report.

[NOTE]
====
Cross-gateway conformance monitoring is available only in the enhanced MuleSoft experience. For detailed steps, see xref:general::exp-governance-monitor-cross-gateway-conformance.adoc[].
====
...
API Governance provides a cross-gateway policy audit export to support compliance audits (such as SOC 2 or GDPR) by consolidating policy application evidence across all connected providers and MuleSoft-native APIs.

[NOTE]
====
The cross-gateway policy audit export is available only in the enhanced MuleSoft experience. For the export procedure and CSV schema, see xref:general::exp-governance-cross-gateway-policy-audit-export.adoc[].
====
