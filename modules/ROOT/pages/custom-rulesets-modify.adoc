...
. Clone it and modify the rules using Anypoint CLI or by copying it and modifying it manually.
. Optionally list the rules in the "after" ruleset.
. Validate the custom ruleset.
. Generate the documentation for the ruleset.
. Publish the customized ruleset along with its associated documentation as a new ruleset asset. 
