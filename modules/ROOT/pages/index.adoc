...
* *Ensure consistent governance of APIs beyond Anypoint Platform:* Govern APIs from systems other than Anypoint Platform using API Discovery.
* *Increase API reuse and reduce duplication:* Enable true centralized API consumption by publishing external APIs in Anypoint Platform.
...
// Screenshot of the API Governance console with callouts

include::_partials/api-gov-console-scap-callouts.adoc[]

== API Discovery Console

In the API Dicovery console, add APIs from external systems and publish and govern them in Anypoint Platform.

// Screenshot of the API Discovery console with callouts

include::_partials/api-discovery-console-scaps-callouts.adoc[tag=api-discovery-console]
...
* In API Manager, you can check API instance conformance by viewing comprehensive validation reports. 

* In Exchange:
** You can view published API specifications' conformance status details, discover provided rulesets, and publish custom rulesets. 
** You can view the ruleset that an instance failed for details on how to fix API instance conformance issues.

* In Design Center API Designer, you can check API specification conformance in the API design phase by applying governance rulesets directly to API specifications as dependencies. Use the *Project Errors* section for information on where the issue is in the specification and how to fix it.
