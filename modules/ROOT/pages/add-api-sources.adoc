...
== Before You Begin to Add AWS Sources
...
Only AWS is available for the pilot.

[[authenticate]]
=== Authenticate to AWS

Authenticate to AWS using your
access key ID, secret access key, and region. You must configure these in AWS before adding them in API Disovery. See <<tips-for-aws-auth>>.
...
Specify additional general information, such as whether to scan immediately to discover external APIs for this external source. 
...
For detailed documentation on configuring IAM access keys, refer to https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html[Managing access keys for IAM users^] in the AWS documentation.

// Get other tips to add

// == Troublehoot Add Sources
