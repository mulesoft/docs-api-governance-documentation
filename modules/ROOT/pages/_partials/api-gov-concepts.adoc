...
A governance profile applies chosen governance rulesets to a selected group of APIs. The API definitions are validated
...
APIs are governed if they are identified by the selection criteria of at least one of the governance profiles. If an API is governed, all versions of that API are considered one governed API. 

API Governance subscription limits are based on your organization's purchased capacity. The API Governance console gives information on usage and displays alerts when you are nearing or exceeding your subscription capacity. Email notifications are automatically sent to Anypoint Platform administrators when your organization exceeds governance subscription capacity.   
