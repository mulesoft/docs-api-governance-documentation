...
* The API is targeted by one or more active profiles.
* There is a valid contact email address configured for the selected recipient.
* An API's conformance status changes to fail because of a change in the API or applied rulesets.
* It's the first time a specific version of an API fails conformance to a specific ruleset. 
...
. In the API Governance console, while creating or updating an active profile, use the *Next* and *Previous* buttons to navigate to the *Notifications* page of the profile for which you want to enable notifications. 
