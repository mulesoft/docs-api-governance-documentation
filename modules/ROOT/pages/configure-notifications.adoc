...
You can send notifications automatically by configuring them in the Governance console or by using Anypoint CLI. You can also send emails manually from the Governance console.
...
. In the Governance console, while creating or updating an active profile, use the *Next* and *Previous* buttons to navigate to the *Notifications* page of the profile you want to enable notifications for. 
. If it's not already selected, select *Enabled* for *Nonconformance notification*.
. Select a recipient:
+
...
To manually notify service owners for any reason, such as when a service is not conformant or you made changes to the profile, you can send them an email notification directly.

To manually send a notification email to a service owner:

. In the Governance console:
* To send a notification email to the service owner from a profile, select a governance profile.
* To send a notification email about a specific service's conformance, select the *Services* tab and view the actions on that service's summary line.
