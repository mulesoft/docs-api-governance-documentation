...
The *Governance Viewer* permission is overridden by higher level permissions. For example, if a user has either the *Governance Administrator* permission or is an organization administrator, setting the *Governance Viewer* permission for them does not limit their permissions.
