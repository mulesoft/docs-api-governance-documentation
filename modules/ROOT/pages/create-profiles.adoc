...
Universal policies:: Configure a policy once and apply it directly to one or more selected API instances. This policy supports third-party providers such as Google Apigee, Azure API Management, and Kong Gateway. The system uses the configuration only to create a native policy on each selected instance. After creation, each policy is independent. Editing the configuration of one policy doesn't affect policies on other instances.
+
[NOTE]
====
