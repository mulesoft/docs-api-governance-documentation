...
Policy Library:: Use this approach to browse available policies and apply one directly as a global policy to selected service instances without creating a full governance profile. This approach is best for targeted or supplemental policy application, especially for services discovered from third-party providers.
+
[NOTE]
====
The *Policy Library* is available only in the enhanced MuleSoft experience. For the complete workflow, see xref:general::exp-governance-policy-library-apply.adoc[].
====
...
If you have connected third-party gateway providers (Kong, Apigee, AWS API Gateway, or Akamai), use this filter to scope the profile to APIs discovered from a specific provider.
+
[NOTE]
====
The *Gateway/Provider* filter is available only in the enhanced MuleSoft experience. For details on connecting providers and using this filter, see xref:general::exp-governance-create-profiles-gateway-provider-filter.adoc[].
====
