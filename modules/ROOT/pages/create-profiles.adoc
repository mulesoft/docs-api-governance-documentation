...
// AUG 13 (POLICY WRITE SCOPE): The Policy Library apply workflow ships Aug 13, not Jul 16.
// The entire "Profiles vs. Policy Library" comparison section is premature until then.
// Uncomment when Policy Write ships Aug 13.
////
...
Policy Library:: Use this approach to browse available policies for selected service instances without creating a full governance profile. This approach is best for targeted or supplemental policy management, especially for services discovered from third-party providers.
+
[NOTE]
====
...
// REMOVED: Gateway/Provider filter for profile scoping does not exist in the strategy creation wizard.
// The code scopes strategies by service types, tags, categories, and runtime only — no provider selector.
// The xref target (exp-governance-create-profiles-gateway-provider-filter.adoc) was deleted in docs-general.
// Do not restore without confirming a specific release milestone with engineering.
