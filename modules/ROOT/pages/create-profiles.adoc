...
* *Rulesets*: Select one or more rulesets you want to use to govern your APIs. The latest ruleset version is selected by default for each API, and you can select a different version. If you select *Automatically use the latest versions of rulesets*, the profile automatically uses the new version as soon as it is published. Otherwise, the profile continues to use the selected versions unless you edit the profile and update the versions. 
