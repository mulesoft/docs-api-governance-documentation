...
* *Rulesets*: Select one or more rulesets you want to use to govern your APIs. You can select a ruleset version other than the latest, which is shown by default. 
** If you select *Latest* for a ruleset's version, the profile automatically uses the latest version of that ruleset if any are published after the profile is saved. This ensures that your APIs selected in this profile are always validated against the latest version of rulesets without the need to manually update the profile. 
** If you select a specific version of a ruleset, the profile continues to use that version even if new versions of the rulesets are released after the profile is saved. This ensures that APIs selected in this profile that are conformant to a specific ruleset version are not inadvertently made nonconformant by a newly published version of the ruleset. You can update the profile and select a new version when you are ready to adopt the new ruleset version.
