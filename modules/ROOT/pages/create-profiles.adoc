...
[NOTE]
====
*Governance profiles + rulesets vs. Policy Library direct apply*

API Governance provides two complementary approaches to policy management. The *Policy Library* approach (*Platform* > *Policy Library*) is available only in the enhanced MuleSoft experience. See xref:general::exp-governance-work-with-strategies.adoc[].

* *Governance profiles with rulesets* — Use this approach to continuously validate API specifications and instances against a defined set of rules, track conformance over time, and receive notifications. This is the recommended approach for ongoing governance programs.
* *Policy Library* (*Platform* > *Policy Library*) — Use this approach to browse available policies and apply one directly as a global policy to selected service instances without creating a full governance profile. The apply workflow follows these steps: Select Policy → Configure Policy → Select Instances → Review & Apply. This approach is best for targeted or supplemental policy application, especially for services discovered from third-party providers.

You can use both approaches together: govern APIs with profiles for continuous conformance tracking, and use the Policy Library for direct policy application to gateway instances.
====

[[create-profile-in-console]]
== Create a Governance Profile Using the Governance Console

The Governance console UI guides you through creating profiles. You can save a profile as draft to test your settings and activate it later.

CAUTION: If you don't select filter criteria, your governance profile applies rulesets to _all_ of your APIs in Exchange.
...
.To create a governance profile using the Governance console:
...
[NOTE]
====
The *Gateway/Provider* filter and *Platform* > *Providers* connectivity described below are available only in the enhanced MuleSoft experience. See xref:general::exp-governance-work-with-strategies.adoc[].
====

* Gateway/Provider
+
If you have connected third-party gateway providers (Kong, Apigee, AWS API Gateway, or Akamai), use this filter to scope the profile to APIs discovered from a specific provider. Select one or more providers from the list of connected providers.
+
NOTE: APIs discovered from third-party providers must be connected via *Platform* > *Providers* before they appear in this filter. See xref:get-started.adoc#third-party-gateway-prereqs[Set Up Third-Party Gateway Connectivity].
...
== Update a Governance Profile Using the Governance Console

The Governance console UI guides you through updating profiles.

To update governance profile information using the console:

. In the Governance console *Profiles* tab, click *Draft* or *Active*.
...
. In the Governance console *Profiles* tab, click *Draft* to show draft profiles.
...
== Delete a Governance Profile Using the Governance Console

To delete a governance profile:

. In the Governance console *Profiles* tab, click the more options menu icon (image:overflow-menu-icon.png[width=9,height=15,fit=line]) for the profile to update and select *Delete*.
. Click *Yes, Delete*.
