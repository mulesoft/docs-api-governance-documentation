...
== Governance Profiles and Rulesets Versus Policy Library Direct Apply

API Governance provides two complementary approaches to policy management. 
...
Policy Library:: Use this approach to browse available policies and apply one directly as a global policy to selected service instances without creating a full governance profile. Access *Policy Library* from *Governance* > *Governance Strategies* > *Policy Library*. The apply workflow follows these steps:
. *Select Policy*
. *Configure Policy*
. *Select Instances*
. *Review & Apply*
+
This approach is best for targeted or supplemental policy application, especially for services discovered from third-party providers. The Policy Library approach is available only in the enhanced MuleSoft experience. See xref:general::exp-governance-work-with-strategies.adoc[].

Use both approaches together to govern APIs with profiles for continuous conformance tracking. Use the Policy Library approach for direct policy application to gateway instances.
