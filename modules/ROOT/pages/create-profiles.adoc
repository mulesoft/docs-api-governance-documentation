...
Automated policies:: Automatically attach a policy to every API that matches a set of criteria, such as runtime, technology, or environment. Policies are applied by rule as APIs move in and out of scope. Automated policies apply only to Anypoint API instances (Flex Gateway and Mule Gateway), not to instances on third-party providers.
Universal policies:: Configure a policy once and apply it directly to one or more API instances that you select, including instances on third-party providers such as Google Apigee, Azure API Management, and Kong Gateway. Use this approach for targeted policy application.
+
[NOTE]
====
