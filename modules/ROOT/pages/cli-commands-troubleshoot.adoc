...
+
The `exchange.json` file is optional when uploading an asset because the CLI includes or infers required properties from parameters, flags, and options. If you download a ruleset project on which to base your custom ruleset, do one of the following before uploading your custom ruleset:
+
...
Anypoint CLI parses commands for specific information and if the CLI doesn't recognize or correctly interpret in the command, the error message might be misleading or vague. 
...
* Ensure that you are entering your information in option values. You can't run most command examples verbatim. You must use your values for parameters and options, including organization ID, environment ID, host ID, asset identifier GAV (group ID/asset ID/version), and folder and file names.
