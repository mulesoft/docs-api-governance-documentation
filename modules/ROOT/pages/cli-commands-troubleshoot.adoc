...
If you get the error `Request failed with status code 409` when using the `exchange asset upload` command to publish a ruleset, it might indicate that you're attempting to upload a duplicate asset.
...
If you get the error `Could not determine asset type, step: Publishing asset` when using the `exchange asset upload` command to publish a ruleset or other asset, Anypoint CLI isn't able to determine the asset type from the command and any properties you provided. Anypoint CLI might not be able to correctly determine the asset type if you :

* Specify a `--type` flag when specifying more than one file type in one or more `--files` flags. 
+
